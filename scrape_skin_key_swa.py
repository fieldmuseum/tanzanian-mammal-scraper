...
def get_child_urls(url:str, output_list:list, base_url:str, run_count:int) -> list:
    ...
    # if run_count is None or run_count > 8: return
    run_count += 1
    ...
    if len(soup_child_list) > 0:
        ...
        if len(page_text_chunks) > 0:
            for chunk in page_text_chunks:
                ...
                if 'width' in chunk.attrs:
                    if chunk['width'] == '200':
                        if re.match(r'Chaguo (A|B)', chunk.text) is not None:
        ...
        # # test batch
        # if run_count is None or run_count > 7: run_count = 100
...
def main():
    ...
    with open(f"{output_path}scraped_skin_key_Swa_{date_suffix}.csv", encoding='utf-8', mode='w') as scraped_urls:
